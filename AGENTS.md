...
| `bun test` | `bun test` (115 tests, bun:test — node:test compatible) |
...
- `extensions/index.ts` — entry. Registers `delegate` tool (primary) + `claude_delegate` alias, and `/delegate` command (primary) + `/claude`, `/codex`, `/opencode`, `/amp`, `/omp` aliases. Reads `delegate` config (with legacy `claudeDelegate` migration), builds prompts, writes partitioned transcripts, renders live feed. `delegate()` is the shared engine — single source of truth for one harness run; everything else (fan-out, verify) is built on top of it, never around it.
  - **Verify**: after `delegate()` gets a result back from the harness (success or error-with-output), it resolves a verify command (`resolveVerifyCommand(opts.verify, template.verify)` — per-call override wins) and, if one is configured, runs it host-side via `runVerify()` (`pi.exec('sh', ['-c', command], {cwd, timeout: VERIFY_TIMEOUT_MS})` — never delegated to the harness). The `VerifyResult` is report-only: appended as its own `### Verify: \`cmd\`` section in the transcript (`buildTranscript`) and injected report (`buildReportContent`), and surfaced whole in the tool result's `details.verify` — it never touches `result.isError`. Skipped entirely when no command is configured (no invented default).
  - **Fan-out** (`harness: "all"` or a comma list, e.g. `"claude,codex"`): detected via `isFanoutSpec()` (command.ts). `resolveHarnessList()` (command.ts, pure) resolves the spec against `detectAll()` — `all` → every *detected* harness, an explicit list is alias/known-validated and also detection-filtered — uninstalled/unknown names are reported (`skipped`/`unknown`) rather than failing the run. The tool path (`runFanoutTool`) and the `/delegate` command path (`runFanoutCommand`, using `runOneDelegation` for the per-harness progress window) both then call `delegate()` **once per resolved harness, sequentially** (a plain `for` loop — this is what "respects `maxConcurrent`" means in practice: only one call is ever in flight, so the existing per-call concurrency guard is never bypassed). Each run writes its own transcript via the existing per-harness `outputsDir`. Results are collected into `FanoutRunSummary[]` and mechanically assembled — no second model call — into one comparison report via `buildFanoutReport()` (activity.ts), which reuses `aggregateSpend`/`formatSpend` for the total-spend rollup. A single-harness call is untouched by any of this — fan-out is purely additive, gated on `isFanoutSpec`.
  - **Notifications**: `/delegate all …` batches successful per-harness completions through `NotifyBatcher` (`extensions/notify.ts`) so the fan-out emits one debounced notification instead of one per harness; failures call `.failure()`, which flushes any pending batch and emits immediately — never delayed or batched. Confined to the command handler's `ctx.ui.notify` path, guarded by `ctx.hasUI`; not a general event system.
...
- **Verify is report-only.** A `verify` command (frontmatter `verify:` or the `--verify=`/`verify` override) runs host-side after the harness exits and its pass/fail is recorded alongside the result — it must never flip `isError`, which stays exactly what the harness itself reported. Never invent a default verify command for a template.
- **Fan-out never bypasses the concurrency guard.** `harness: "all"`/a comma list runs the existing `delegate()` engine once per resolved harness in a plain sequential loop — do not `Promise.all` them or otherwise run more than one at a time, since `delegate()`'s own concurrency check *rejects* (throws) rather than queues.
