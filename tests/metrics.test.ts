...
test('formatMetrics includes all fields', () => {
	assert.equal(formatMetrics({ numTurns: 3, totalCostUsd: 0.5432, promptTokens: 62000, contextPercent: 6.2, durationMs: 12500 }), '3 turn(s) · $0.543 · 62k tok · 6.2% ctx · 13s');
});
...
test('parseTranscriptMeta extracts mode, cost, session', () => {
	const head = ['# Delegated Claude run — review', '', '- mode: review', '- cost: $0.7244 · isError: false', '- session: b2125252-aaaa-bbbb-cccc-dddddddddddd', '', '## Activity'].join('\n');
	assert.deepEqual(parseTranscriptMeta(head), { mode: 'review', cost: 0.7244, sessionId: 'b2125252-aaaa-bbbb-cccc-dddddddddddd', harness: 'claude' });
});
...
test('buildReportContent assembles header, body and footers', () => {
	const content = buildReportContent({ harness: 'claude', mode: 'review', metrics: '3 turn(s) · $0.543', body: 'Findings…', file: '/tmp/out.md', sessionId: 'abc-123' });
